...
```go
esvsdkgo.WithServerURL("https://api.example.com")
```
...
```go
esvsdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
esvsdkgo.WithClient(httpClient)
```
...
```go
esvsdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
esvsdkgo.WithTimeout(30 * time.Second)
```
